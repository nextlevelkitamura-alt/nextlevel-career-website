...
function getSlotUrl(
  ...
  const url =
    selectedDate?.bookingUrl?.trim() || option?.bookingUrl?.trim() || "";
...
export default function ConsultationBookingSlots({
  ...
  const canNavigate = Boolean(
    route && option && selectedDate?.status === "available" && slots.length > 0,
  );
  const slotTitle = selectedDate?.slotTitle || "働き方を相談";
  const slotDescription =
    selectedDate?.slotDescription || "新宿で直接相談したい方";
      ...
      >
        <div className="grid grid-cols-[minmax(0,1fr)_148px] gap-3 sm:grid-cols-[minmax(0,1fr)_190px] sm:gap-4">
          ...
          <div className="flex min-w-0 flex-col justify-center">
            <p
              className={cn(
                "mb-1.5 text-center text-[11px] font-extrabold tracking-normal sm:text-xs",
                theme.slotLabelClassName,
              )}
            >
              予約はこちらから
            </p>
