...
// Check if parse failed (value exists but couldn't extract amounts)
function isParseFailure(value: string, parsed: { minAmount: string }): boolean {
    return !!value.trim() && !parsed.minAmount;
}
...
export default function SalaryInput({ value, onChange }: SalaryInputProps) {
    ...
    useEffect(() => {
        ...
        if (isParseFailure(value, parsed)) {
            ...
            // Don't auto-exit rawMode here — let user choose via button
        }
    }, [value]);
    ...
    // Raw text edit mode (when parse fails or manually toggled)
    if (rawMode) {
        return (
