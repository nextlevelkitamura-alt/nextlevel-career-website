"use client";

import { useEffect, useState } from "react";
import { createClient, hasBrowserSupabaseEnv } from "@/utils/supabase/client";
import { getAdminNotificationCounts } from "@/app/admin/actions";

interface AdminNavBadgeProps {
    type: "applications" | "inquiries" | "users" | "consultations";
}

export default function AdminNavBadge({ type }: AdminNavBadgeProps) {
    const [count, setCount] = useState(0);
    const supabase = hasBrowserSupabaseEnv() ? createClient() : null;

    useEffect(() => {
        // Initial fetch
        const fetchInitialCounts = async () => {
            const counts = await getAdminNotificationCounts();
            setCount(type === "applications" ? counts.applications : type === "inquiries" ? counts.inquiries : type === "consultations" ? counts.consultations : counts.users);
        };
        fetchInitialCounts();

        if (!supabase) {
            if (!hasBrowserSupabaseEnv()) {
                console.warn("Admin nav badge subscriptions disabled: NEXT_PUBLIC_SUPABASE_* is missing in the browser bundle.");
            }
            return;
        }

        const resourceChannel = supabase
            .channel(`admin-nav-${type}-new`)
            .on(
                'postgres_changes',
                { event: 'INSERT', schema: 'public', table: type === "applications" ? 'applications' : type === "inquiries" ? 'client_inquiries' : type === "consultations" ? 'consultation_bookings' : 'profiles' },
                () => fetchInitialCounts()
            )
            .subscribe();

        const readsChannel = supabase
            .channel(`admin-nav-${type}-reads`)
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'admin_notification_reads' },
                () => fetchInitialCounts()
            )
            .subscribe();

        return () => {
            supabase.removeChannel(resourceChannel);
            supabase.removeChannel(readsChannel);
        };
    }, [type, supabase]);

    if (count === 0) return null;

    return (
        <span className="ml-1.5 inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-bold leading-none text-white bg-red-600 rounded-full animate-in fade-in zoom-in duration-300">
            {count > 99 ? "99+" : count}
        </span>
    );
}
