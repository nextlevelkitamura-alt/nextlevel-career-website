...
function normalizeNumericText(value: unknown): string | undefined {
...
function normalizeWorkDaysPerWeek(value: unknown): string | undefined {
    const text = coerceOptionalString(value);
    if (!text) return text;
    // "週4~5日" -> "4~5", "週5日" -> "5"
    const trimmed = text.trim().replace(/^週\s*/, "").replace(/\s*日$/, "");
    return trimmed || text.trim();
}

function normalizeDispatchEndDate(value: unknown): string | undefined {
    const text = coerceOptionalString(value);
    if (!text) return text;
...
- **時給1,500円以上**の場合は、「高時給」のような抽象語よりも **実際の時給額（例: 時給1650円）** を優先して見せてください。
- **駅から徒歩10分以内の場合**は「駅チカ」とだけ書くより、確定している駅名やエリア名を優先してください。
- 両方に該当する場合も、過度な装飾ではなく業務が想像できるタイトルにしてください。
- タイトルに含める情報は、求人データに実際に存在する項目のみとしてください。
