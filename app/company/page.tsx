...
const companyItems: CompanyItem[] = [
    ...
    },
    {
        label: 'メールアドレス',
        value: (
            <a href="mailto:info@e-nextlevel.jp" className="text-sky-700 hover:text-sky-800 underline underline-offset-2">
                info@e-nextlevel.jp
            </a>
        ),
    },
            ...
            <ul className="space-y-2">
                {businessItems.map((item) => (
                    <li key={item} className="flex items-start gap-2">
                        <span className="mt-1 h-2 w-2 shrink-0 rotate-45 bg-sky-500" />
                        <span>{item}</span>
                    </li>
                ))}
            </ul>
            ...
            <div className="relative mx-auto w-full max-w-6xl px-4">
                <div className="rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-[0_24px_80px_-40px_rgba(15,23,42,0.45)] backdrop-blur md:p-10">
                    <div className="flex items-center gap-5 pb-6 md:gap-8 md:pb-8">
                        ...
                        <h1 className="text-3xl font-bold tracking-tight text-sky-500 md:text-5xl">会社概要</h1>
                        <span className="h-px flex-1 bg-sky-400/80" />
                    </div>

                    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                        <dl className="divide-y divide-dashed divide-slate-300">
                            {companyItems.map((item) => (
                                <div key={item.label} className="grid gap-3 px-5 py-5 md:grid-cols-[220px_1fr] md:gap-8 md:px-8 md:py-7">
                                    <dt className="text-xl font-bold text-slate-800">{item.label}</dt>
                                    <dd className="text-lg leading-8 text-slate-700">{item.value}</dd>
                                </div>
                            ))}
                        </dl>
                    </div>
                    ...
                    <div className="mt-8 text-center">
                        <Link
                            href="/"
                            className="inline-flex items-center justify-center rounded-full bg-slate-900 px-6 py-3 text-base font-medium text-white shadow-sm transition-colors hover:bg-slate-800"
                        >
                            トップページに戻る
                        </Link>
                    </div>
