-- =============================================================
-- Fix: category カラムの JSON文字列問題を修正
-- 原因: category が text[] なのに '["販売・接客"]' のような
--       JSON文字列が格納されていた
-- =============================================================

-- 1. 既存データ修正: 配列要素内にJSON文字列が入っている場合を修正
-- text[] の中身が '["販売・接客"]' のようなJSON配列文字列になっている
UPDATE jobs
SET category = ARRAY(SELECT jsonb_array_elements_text(elem::jsonb))
FROM (
  SELECT DISTINCT id, unnest(category) AS elem
  FROM jobs
...
-- 2. create_job_with_details: category を tags と同様に配列として処理
...
BEGIN
  ...
  VALUES (
    ...
    p_job->>'salary',
    CASE
      WHEN p_job ? 'category' THEN ARRAY(SELECT jsonb_array_elements_text(p_job->'category'))
      ELSE ARRAY[]::text[]
    END,
    CASE
...
-- 3. update_job_with_details: category を tags と同様に配列として処理
