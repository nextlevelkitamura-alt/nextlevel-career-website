...
| 週 | タスク | 工数 |
|:---:|-------|:---:|
| W11 (3/10-16) | ドキュメント整備 + 人生管理リポ統合 | 2h |
| W12 (3/17-23) | actions.ts分割リファクタリング | 4h |
| W13 (3/24-31) | AI統一 Phase 1-2 着手（差分プレビューコンポーネント） | 6h |

**マイルストーン (3/31)**: ドキュメント最新化、管理体系に組み込み済み、コードベースがモジュール化

### 4月 — 管理機能の完成
| 週 | タスク | 工数 |
|:---:|-------|:---:|
| W14 (4/1-6) | AI統一 Phase 2-3 完了 | 4h |
| W15 (4/7-13) | AI統一 Phase 4（データ堅牢化）+ DraftJobEditor正社員フィールドUI | 6h |
| W16 (4/14-20) | クリック分析ドリルダウン Phase 1-2 | 4h |
| W17 (4/21-27) | クリック分析 Phase 3-4 + メール通知（企業問い合わせ時） | 5h |

**マイルストーン (4/30)**: AI抽出体験統一、データ堅牢化、分析機能完成、メール通知

### 5月 — ユーザー体験向上 + 品質保証
| 週 | タスク | 工数 |
|:---:|-------|:---:|
| W18 (5/1-11) | ヘッダーUI改善（Gemini引き継ぎ分の仕上げ） | 3h |
| W19 (5/12-18) | お気に入り機能 | 4h |
| W20 (5/19-25) | シェア機能 + OGP最適化 | 3h |
| W21 (5/26-31) | E2Eテスト基盤（主要3フロー） | 6h |

**マイルストーン (5/31)**: ユーザー向け機能充実、UI刷新、E2Eテスト基盤 → 6月から保守モードへ
...
#### ⏳ ヘッダーUI改善 (G) → Gemini 実装中
- 状態: Gemini に引き継ぎ済み
- 内容: ヘッダーナビゲーションをシンプル・クリーンなデザインにリニューアル
...
### 2026-03-10
- サイトアイコンをブランドファビコンセットに統一
- ドメインを nextlevelcareer-official.com に切り替え（3/5）
- ヘッダーロゴをカスタムブランドアセットに更新（3/7-8）
- タブアイコンをアプリアイコンに変更（3/9）
- publicSite.ts導入（認証コンテキスト・バナー取得の共通化）+ SSGバグ修正（force-dynamic）
- 人生管理リポとの統合（プロジェクト一覧・計画一覧に登録）
- 3ヶ月ロードマップ策定（3月=足場固め、4月=管理機能完成、5月=UX+品質）

### 2026-02-27
- 会社概要セクションのモバイル読みやすさ改善
- キャリアコンサルティングPDFへのリンク追加
- 会社ページデザインをオレンジテーマに統一
- 認証フロー・ルーティング改善、UIコピー/レイアウト調整
- ホームページのメッセージング・機能カードコピー更新
- LP コピー更新・ワークスペース整備
- 機能カードレイアウトスペーシング調整
- バナーカルーセル機能追加 & トップページ微調整
- トップページ・会社概要の微調整
- 福利厚生の紐付けを取引先管理に統合（リファクタ）
- 掲載元別デフォルト福利厚生の自動付与機能
