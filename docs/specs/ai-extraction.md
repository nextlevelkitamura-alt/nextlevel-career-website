...
### 概要
AI抽出結果をフォームに反映する際、全フィールド一括上書きではなく、**差分プレビュー**を表示してフィールド単位で適用/スキップを選択できるUI。

### 対象画面
- 新規作成画面（`/admin/jobs/create`）
- 編集画面（`/admin/jobs/[id]/edit`）

### 操作フロー

| ステップ | 操作 | 説明 |
|----------|------|------|
| 1 | ファイル選択 | アップロード済み or 事前登録ファイルを選択 |
| 2 | AI抽出実行 | AiExtractButtonまたはFileUploaderの「AI読込」をクリック |
| 3 | 差分プレビュー表示 | 現在値 vs AI抽出値をフィールドごとに比較表示 |
| 4 | フィールド選択 | チェックボックスで適用するフィールドを選択 |
| 5 | 適用 | 選択したフィールドのみフォームに反映 |

### 差分プレビューの表示ルール

| 状態 | 表示 | デフォルト選択 |
|------|------|---------------|
| 空 → 値あり（新規追加） | 「（空）→ 新しい値」 | ✅ チェック済み |
| 値あり → 異なる値（変更） | 「前の値 → 新しい値」 | ✅ チェック済み |
| 値あり → 空（削除） | 「前の値 → （空）」 | ☐ 未チェック |
| 値あり → 同じ値（変更なし） | 折りたたんで非表示 | — |
...
| 全選択ボタン | すべてのフィールドを✅に |
| 全解除ボタン | すべてのフィールドを☐に |
| 適用ボタン | 選択フィールドのみフォームに反映 |
| キャンセルボタン | 差分プレビューを閉じる（変更なし） |
| 変更数カウンター | 「12項目中8項目を適用」等の表示 |

### 編集画面固有の仕様

| 項目 | 仕様 |
|------|------|
| AiExtractButton | 新規作成と同じコンポーネントを配置 |
| FileUploaderの「AI読込」 | 差分プレビュー経由に変更（直接上書きしない） |
| 事前登録ファイル選択 | DraftFileSelectorを編集画面にも追加 |
| 雇用形態モード | DB値から自動判定（手動切替不要） |

### エッジケース

| ケース | 挙動 |
|--------|------|
| フォームが完全に空（初回抽出） | 全フィールド自動選択で差分プレビュー表示 |
| AI抽出エラー | エラーメッセージを表示、差分プレビューは出ない |
| 抽出結果が空（何も抽出されない） | 「抽出結果がありません」メッセージを表示 |
| タグ系フィールド（holidays, benefits） | JSON配列として比較、要素単位での差分 |
...
| 項目 | 現状 | 改善後 |
|------|------|--------|
| fulltime_job_details | 値がある場合のみINSERT | 雇用形態が正社員なら必ずINSERT（空でも行作成） |
| dispatch_job_details | 値がある場合のみINSERT | 雇用形態が派遣なら必ずINSERT（空でも行作成） |

### updateJob の改善

| 項目 | 現状 | 改善後 |
|------|------|--------|
| 詳細テーブル未存在 | UPSERTが失敗する可能性 | 存在チェック→なければINSERT、あればUPDATE |

### 編集画面の「詳細データ未保存」警告

| 項目 | 現状 | 改善後 |
|------|------|--------|
| 警告表示 | テキストのみ | 警告 + 「空のレコードを作成」ボタン |
| 自動修復 | なし | ボタン押下で詳細テーブルに空行をINSERT |
...
|------|------|
| 2026-02-13 | 初版作成 |
| 2026-02-21 | v2: 差分プレビューUI仕様追加、データ保存堅牢化仕様追加、編集画面AI統一仕様追加 |
